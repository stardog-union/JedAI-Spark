...
object StatisticsEstimator {
  ...
  /** *
    ...
    * */
  def estimateNormalizedEntropy(blocks: RDD[BlockAbstract], profileBlocks: Broadcast[Map[Long, ProfileBlocks]]): RDD[(Int, Double)] = {
  ...
  /** *
    * Estimates the blocks entropy
    * */
  def estimateEntropy(blocks: RDD[BlockAbstract], profileBlocks: Broadcast[Map[Long, ProfileBlocks]]): RDD[(Int, Double)] = {
    val keysPerBlock = blocks.map {
      block =>
        val allKeys = block.getAllProfiles.flatMap {
          profileID =>
