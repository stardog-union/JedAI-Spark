  ...
  (RDD[BlockAbstract], scala.collection.Map[String, Map[Int, Iterable[String]]]) = {
    ...
    val blocks: RDD[BlockAbstract] = profilesGroupedWithIds map {
      case ((entityIds, entropy, clusterID, blockingKey), blockId) =>
        if (separatorIDs.isEmpty) BlockDirty(blockId.toInt, entityIds, entropy, clusterID, blockingKey = blockingKey)
        else BlockClean(blockId.toInt, entityIds, entropy, clusterID, blockingKey = blockingKey)
    }
