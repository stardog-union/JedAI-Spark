...
import SparkER.DataStructures.{BlockAbstract, BlockClean, UnweightedEdge}
import SparkER.Utilities.Converters
import org.scalatest.matchers.should.Matchers
import org.scalatest.wordspec.AnyWordSpec

class CEPSpec extends AnyWordSpec with Matchers with SharedSparkContext {
  ...
  "CEP.CEP" should {
    "retain the K edges with the maximum weight" when {
      "the given task is Dirty ER" in {
        ...
        val blocks: RDD[BlockAbstract] = sc.makeRDD(
          Seq(
            BlockClean(blockID = 0, profiles = Array(Set(0, 1, 2, 3, 4))),
            BlockClean(blockID = 1, profiles = Array(Set(0, 1, 2)))
          )
        )
        val actual = CEP.CEP(
